...
@app.route('/')
def display_data():
    # Read the CSV file with extracted coordinates
    extracted_df = pd.read_csv("extracted_coordinates.csv")

    # Convert the extracted coordinates column from string to a list of tuples
    extracted_df["location_coordinates"] = extracted_df["location_coordinates"].apply(eval)

    # Pass the DataFrame to the HTML template for rendering
    return render_template('display_data.html', data=extracted_df.to_dict(orient="records"))
